...
#ifdef __APPLE__
  s21_sprintf(errbuf, "Unknown error: %d", errnum);
#else
  s21_sprintf(errbuf, "Unknown error %d", errnum);
#endif
  return errbuf;
...
char *s21_strtok(char *str, const char *delim) {
  static char *s21_token = s21_NULL;
  char *result = s21_NULL;
  if (str != s21_NULL) {
    s21_token = str;
  }
  ...
  s21_token += s21_strspn(s21_token, delim);
  if (*s21_token == '\0') {
    s21_token = s21_NULL;
  } else {
    char *token_end = s21_token + s21_strcspn(s21_token, delim);
    if (*token_end == '\0') {
      result = s21_token;
      s21_token = s21_NULL;
    } else {
...
void *s21_insert(const char *src, const char *str, s21_size_t start_index) {
  ...
  if (src != NULL && str != NULL) {
    ...
    if (start_index > 0 && start_index < size_src) {
      ...
      for (s21_size_t i = 0, j = 0, k = 0; src[i] != '\0'; i++) {
...
void *s21_trim(const char *src, const char *trim_chars) {
  ...
  if (src != NULL && trim_chars != NULL) {
    while (s21_strchr(trim_chars, *src)) src++;
    asprintf(&result, "%s", src);
    int end = s21_strlen(src) - 1;
    while (s21_strchr(trim_chars, result[end])) {
      result[end--] = '\0';
    }
  }
  return result;
